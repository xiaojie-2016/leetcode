package com.xiaojie.leetcode._008;
...
public class Solution {
    ...
    public int myAtoi(String str) {
        if (str != null) {
            ...
            // 找字符串，因为正则不能用,不过用String类里面的匹配还是可以的
            // error: cannot find symbol: class Matcher
    ...
    private int slu(String numStr) {
        ...
        for (char c : numStr.toCharArray()) {
            if (c == '-') {
                ...
                if (symbol == 1) {
                    int n = num * 10 + Integer.parseInt(String.valueOf(c));
                    if (n / 10 != num) {
